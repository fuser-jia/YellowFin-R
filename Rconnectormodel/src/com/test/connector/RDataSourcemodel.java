...
import com.test.connector.RMetaDatamodel;
...
public class RDataSourcemodel extends AbstractDataSource {
	//Use Yellowfin's logger
	private static final YFLogger log = YFLogger.getLogger(RDataSourcemodel.class.getName());
...
//					c.parseAndEval("try(source(\""+script+"\"),silent=TRUE)");
					...
					for (i = 0; i < dataTemp.size(); i++) {
						for (j = 0; j < columns.size(); j++) {
							...
							System.out.println(dataTemp.get(i)[j]);
						}
					}
